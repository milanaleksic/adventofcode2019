...
//        println!("Field1={:?}", field1);
        ...
        for (cell1, _) in field1 {
            if let Some(latency2) = field2.get(&cell1) {
                let new_distance = cell1.distance();
                best_match_distance = match best_match_distance {
                    Some(m) => {
                        if new_distance < m {
                            Some(new_distance)
                        } else {
                ...
                let new_latency = cell1.latency() + *latency2;
                best_match_latency = match best_match_latency {
                    Some(m) => {
                        if new_latency < m {
                            Some(new_latency)
                        } else {
    ...
    fn walk(&self, cable: Vec<(i32, i32)>) -> HashMap<Cell, i32> {
        ...
        for step in cable {
            if step.0 != 0 {
                let begin = iter.0;
                let end = begin + step.0.abs();
                let incr = if step.0 < 0 { -1 } else { 1 };
                for ix in begin..end {
                    latency += 1;
                    let cell = Cell::new(iter.0 + (ix-begin+1)*incr, iter.1, latency);
            ...
            if step.1 != 0 {
                let begin = iter.1;
                let end = begin + step.1.abs();
                let incr = if step.1 < 0 { -1 } else { 1 };
                for iy in begin..end {
                    latency += 1;
                    let cell = Cell::new(iter.0, iter.1 + (iy-begin+1)*incr, latency);
                    if !field.contains_key(&cell) {
                        field.insert(cell, latency);
                    }
                }
...
impl common::Solver for Solver {
    ...
    fn solve_a(&self) -> String {
        ...
        format!("{}", self.solve_for(cables[0].clone(), cables[1].clone()).0)
    }

    fn solve_b(&self) -> String {
        let cables = self.get_data();
        format!("{}", self.solve_for(cables[0].clone(), cables[1].clone()).1)
    }
}
